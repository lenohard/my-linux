#!/bin/bash
SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
if [[ -z $1 ]];
then
    # readarray -t regexs < $SCRIPT_DIR/regexs
    # length=${#regexs[@]}
    # for (( j=0; j<${length}; j++ ));
    # do
    #     echo "$j ${regexs[j]}"
    # done
    while [[ 1 ]]
    do
        res=$(fzf < $SCRIPT_DIR/regexs)
        echo "$res"
        awk -v p1="$res" 'match($0, p1, m){print m[1]"   ,   "m[2]}' "${2:-result.txt}"
        read -p "continue (Y|C|N):"
        if [[ ${REPLY^^} == "Y" ]];
        then
            REG="$res"
            break;
        fi
    done
else
    REG=$1
    awk -v p1="$REG" 'match($0, p1, m){print m[1]"   ,   "m[2]}' "${2:-result.txt}"
    read -p "OK? (Y|N):"
fi

if [[ $REPLY =~ ^[Yy]$ || -z $REPLY ]]
then
    read -p "RANK: " rank
    awk -v p="$REG" -v rank="$rank" 'NR==1{print p"    "rank}' result.txt >> content
    echo "CURRENT CONTENTL"
    awk -i inplace '!seen[$0]++' content            # remove duplicate lines
    if grep offset content >> /dev/null 2>&1;
    then
        echo "offset exist"
    else
        read -p "OFFSET ? : " offset
        awk -i inplace -v ost="$offset" 'BEGINFILE{print "offset    "ost}{print}' content
    fi

    if grep "$REG" ~/kit/ocr/readme -F >> /dev/null 2>&1;
    then
        echo "pattern exist"
    else
        echo "$REG" >> ~/kit/ocr/regexs
    fi
else
    exit
fi
